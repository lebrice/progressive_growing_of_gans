...
from gaussian_blur import gaussian_blur, GaussianBlur2D, appropriate_kernel_size, image_at_scale
...
def main():
    with tf.Graph().as_default():
        ...
        scale = tf.Variable(0.0, trainable=False)
        ...
        _blurred_images = image_at_scale(
            _images,
            scale,
        )
        ...
        with tf.Session() as sess:
            ...
            for i, factor in enumerate(np.arange(1, 0, -0.1)):
                ...
                blurred_image, t = sess.run(
                    # [_blurred_images, _blur_std, _kernel_size],
                    [_blurred_images, scale],
                    feed_dict={
                ...
                axeslist.ravel()[i].set_title("original" if i == 0 else f"scale: {t}")
